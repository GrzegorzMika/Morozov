            ...
            for _ in range(replications):
                try:
                    ...
                    tsvd = TSVD(kernel=kernel, singular_values=spectrum.singular_values,
                                ...
                                observations=obs, sample_size=s, max_size=max_size, transformed_measure=True, njobs=3)
                    tsvd.estimate()
                    tsvd.oracle(fun)
                    ...
                    pass
            pd.DataFrame(results).to_csv('TransformedMeasure_TSVD_{}_{}.csv'.format(functions_name[i], s))
