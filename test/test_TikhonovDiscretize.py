...
estimator = Tikhonov(kernel=identity, lower=0, upper=1, grid_size=100, observations=observations, sample_size=50)
...
class TestAttributes:
    ...
    def test_order(self):
        assert hasattr(estimator, 'order')
        assert estimator.order == 1
        estimator_tmp = Tikhonov(kernel=identity, lower=0, upper=1, grid_size=100,
                                 observations=observations, sample_size=50, order=50)
        assert estimator_tmp.order == 50
    ...
    def test_parameter_space_size(self):
        assert hasattr(estimator, 'parameter_space_size')
        assert estimator.parameter_space_size == 100
        estimator_tmp = Tikhonov(kernel=identity, lower=0, upper=1, grid_size=100,
                                 observations=observations, sample_size=50, parameter_space_size=123)
        assert estimator_tmp.parameter_space_size == 123
